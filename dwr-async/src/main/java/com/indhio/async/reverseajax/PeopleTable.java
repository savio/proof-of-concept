...
import com.indhio.async.people.Person;
...
public class PeopleTable implements Runnable {
	...
	/**
	 * Called from the client to add an attribute on the ScriptSession. This attribute will be used so that only pages (ScriptSessions) that have set this attribute will be updated.
	 */
	...
	/**
	 * Called from the client to remove an attribute from the ScriptSession. When called from a client that client will no longer receive updates (unless addAttributeToScriptSession) is called again.
	 */
	...
	/**
	 * This is the ScriptSessionFilter that will be used to filter out all ScriptSessions unless they contain the SCRIPT_SESSION_ATTR attribute.
	 */
